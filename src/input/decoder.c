...
static mtime_t DecoderGetDisplayDate( decoder_t *p_dec, mtime_t i_ts )
{
    ...
    if( input_clock_ConvertTS( VLC_OBJECT(p_dec), p_owner->p_clock, NULL, &i_ts, NULL, INT64_MAX, p_dec->fmt_out.i_cat == VIDEO_ES ) ) {
                          ...
                          mtime_t *pi_duration, int *pi_rate, mtime_t i_ts_bound )
{
    ...
    if( *pi_ts0 > VLC_TS_INVALID )
    {
        *pi_ts0 += i_es_delay;
        ...
        if( input_clock_ConvertTS( VLC_OBJECT(p_dec), p_clock, &i_rate, pi_ts0, pi_ts1, i_ts_bound, p_dec->fmt_out.i_cat == VIDEO_ES ) ) {
